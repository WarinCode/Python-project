...
# สร้างเลข id แบบสุ่ม
for item in menu: item["id"] = createID()
...
def searchMenu(param):
    try:
        ...
        if param in foodLi:
            ...
            raise Warning(f'🔴 Error: "{param}" ไม่อยู่ในเมนูอาหารโปรดใส่ชื่ออาหารหรือรหัสสินค้าใหม่อีกครั้ง!')
        return idx
    except ValueError: # เมื่อใส่ชื่อหรือ id ที่ไม่อยู่ในเมนู
        print(f'🔴 Error: "{param}" ไม่อยู่ในเมนูอาหารโปรดใส่ชื่ออาหารหรือรหัสสินค้าใหม่ให้ถูกต้อง!')
...
# ? function แสดงคำสั่ง
# เพิ่มแต่ละ columns
commands.add_column('คำสั่ง', ("e" , "c", "m" , "b", "a" , "d" , "ed" , "cl") )
commands.add_column('ชื่อคำสั่งเต็ม', ("exit" , "commands", "menu" , "buy","add" , "delete" , "edit" , "clear") )
commands.add_column("ความหมายของคำสั่ง" , ("ออกจากโปรแกรม" , "แสดงคำสั่ง" , "แสดงเมนูอาหาร" , "สั่งซื้อสินค้า" , "เพิ่มรายการสินค้า" , "ลบรายการสินค้า" ,"แก้ไขชื่อรายการสินค้า" , "ลบรายการสินค้าทั้งหมด") )
def showCommands():
    print('\nเลือกพิมพ์คำสั่งเหล่านี้เพื่อดำเนินการ'.center(50))
#     print(f"""\tคำสั่ง
# e (exit)      ออกจากโปรแกรม
# c (commands)  แสดงคำสั่ง
# m (menu)      แสดงเมนูอาหาร
# b (buy)       สั่งซื้อสินค้า
# a (add)       เพิ่มรายการสินค้า
# edit (edit)   แก้ไขชื่อรายการสินค้า
# clear (clear) ลบรายการสินค้าทั้งหมด""")
    print(commands,'\n')
showCommands()
        ...
        if isEnd:
            ...
            if order.__len__() != 0: 
                ...
                for number, key in enumerate(order):
                    print(f"🍽 {number + 1}. {key} จำนวน {showOrder[key]:,} อย่าง ราคาจานละ {price_li[number]} บาท รวมเป็นเงิน {order[key]:,} บาท")
    ...
    while True:
        try:
            ...
            if foodName == "e" or foodName == "end": 
                ...
                showMenu()
                continue
            elif (foodName in foodLi) and not(foodName == "e" or foodName == "end") and not(foodName == "m" or foodName == "menu"):
...
def removeItems():
    ...
    while True:
        ...
        try:
            ...
            elif (inp in foodLi) or (int(inp) in idLi): 
                deleteElements(inp)
            elif (inp not in foodLi) and (int(inp) not in idLi):
    ...
    while True:
        try:
            ...
            if inp == 'menu' or inp == 'm':
                ...
                findIndex = searchMenu(inp)
                idx = findIndex
                print(f'คุณเลือกรายการสินค้าที่จะแก้ไข คือ {menu[idx]["name"]} ราคา {menu[idx]["price"]} บาท')
                changeFoodName = input(f'แก้ไขชื่อ จาก "{menu[idx]["name"]}" เป็น -> : ')
                assert not(changeFoodName in foodLi) 
                ...
                # แก้ไขข้อมูล dict
                menu[idx]["name"] = changeFoodName
                menu[idx]["price"] = price
                print('✔ แก้ไขรายการอาหารเสร็จสิ้น')                
                setElements()
        except ValueError:
            print('❌ ราคาสินค้าต้องตั้งเป็นตัวเลขจำนวนเต็มเท่านั้น!')
        except AssertionError:
            print('❌ ชื่อที่คุณทำการแก้ไขนั้นเป็นชื่อซ้ำอยู่ในเมนูอาหารโปรดแก้ไขชื่อที่ไม่เหมือนกัน หรือ ห้ามชื่อว่างเปล่า!')
        except UserWarning as err:
            print(err)
