...
class Register(Console):
    ...
    __user__ = {
        ...
        "AccessPermissions": { # สิทธิ์การเข้าถึงของคำสั่ง ขึ้นอยู่กับตำแหน่งงานของผู้ใช้งานในร้านอาหาร
            "AddData": False, # สิทธิ์ในการเพิ่มข้อมูล
            "ModifyData": False, # สิทธิ์ในการแก้ไขข้อมูล
            "DeleteData": False, # สิทธิ์ในการลบข้อมูล
            "DeleteAllData": False, # สิทธิ์ในการลบข้อมูลทั้งหมด
            "ViewLog": False, # สิทธิ์ในการดูบันทึกของโปรแกรม
            "SellFood": False # สิทธิ์ในการกดสั่งซื้ออาหารให้ลูกค้า
        }
    }
        ...
        def loginFunction() -> Dict[str , Any]:
            ...
            while not bool(userLogin["nameOrEmail"]) or not bool(userLogin["password"]):
                ...
                while not bool(userLogin["password"]):
                    try:
                        # ถ้าใช้ password=True จะสามารถซ่อนการแสดงข้อความที่เป็นรหัสผ่านได้
                        password = self.input("รหัสผ่าน : ").strip()
                        if isEmpty(password):
    ...
    def __createAccount__(self) -> Dict[str , str]:
        ...
        while (not bool(newUser["name"])) or (not bool(newUser["password"])) or (not bool(newUser["position"])) or (not bool(newUser["email"])):
            ...
            self.print('[cyan]กำลังสร้างบัญชีผู้ใช้งาน กรุณารอสักครู่ ████████░░░ [/]')
            # sleep(3)
            self.print('[green bold]✓ สร้างบัญชีผู้ใช้งานสำเร็จโปรด Login เพื่อเข้าใช้งานโปรแกรม[/]')
            self.print('🔹[blue] กลับไปที่หน้า Login[/]')
    ...
    def __logout__(self , callbackFunction: Any) -> bool:
        ...
        if self.input('คุณต้องการออกจากบัญชีผู้ใช้งานนี้ (y/n) : ').lower().strip() == "y":
            ...
            self.print('[cyan]กำลังนำคุณออกจากบัญชี ...[/]')
            # sleep(3)
            self.print('[blue]คุณออกจากบัญชีนี้เรียบร้อย[/]')
        else:
            self.print('[orange1]⚠ คุณยกเลิกการออกจากบัญชีนี้[/]')
        return status

    #? method ในการให้ข้อมูลผู้ใช้งาน 
    def __getUser__(self , callbackFunction: Any) -> Dict[str , str | Union[str , Dict[str , str]]]:
        user = None
        ...
        while not bool(user):
            try:
                ...
                self.print('[red]❌ โปรดพิมพ์เป็นตัวเลขเท่านั้น[/]')
            except Warning as err:
                self.print(err.__str__() , style='red')
        return user
...
class Date(Console):
    ...
    def greeting(self , h: int) -> None:
        ...
        # แสดงข้อความ
        self.print(f'🙏 {hi} คุณ {self.__user__["name"]} วันนี้ วัน{self.days[self.now.date().weekday()]} ที่ {self.now.date().day} เดือน {self.months[self.now.date().month - 1]} ปี พ.ศ. {self.year} ({self.today})')
...
class Configuration(Register , Date , Console):
    ...
    def __setPermissions__(self , user: Dict[str , Any] = None) -> None:
        ...
        if position in self.POSITIONS["management"] or user["name"] == 'root': # ตำแหน่งงานบริหาร
    ...
    def __accessDenide__(self , context: str) -> None:
...
class Program(Configuration , Date , Console): 
    ...
    def showMenu(self):
        ...
        self.print(f'🌟 ตอนนี้ไม่มีรายการสินค้าใดๆโปรดเพิ่มสินค้าก่อนแสดงรายการเมนู!') if self.__menu__.__len__() == 0 else self.print('\n' , self.__menuTable__ , '\n')
            ...
            try:
                ...
                # แสดง order ที่สั่งไป
                    ...
                    if self.__currentOrder__ == []:
                        ...
                        totalAmount = 0
                        count = 0
                        self.print('อาหารที่คุณสั่งไปคือ : ')
                        for item in self.__currentOrder__:
                            count += 1
                            totalAmount += item["amount"]
                            self.print(f'{count}.) {item["name"]} จำนวน {item["amount"]} อย่าง')
                        self.print(f'จำนวนทั้งหมด {totalAmount}')
                ...
                #* เช็คชื่ออาหารหรือรหัสสินค้าว่าอยู่ใน list หรือไม่
                    ...
                    while self.__PROGRAMSTATUS__["isContinue"]:
                        try:
                            ...
                            self.print("[red]❌ โปรดใส่เป็นเลขจำนวนเต็มเท่านั้น![/]")
                        except UserWarning as err:
                            self.print(err.__str__() , style='red')
                        else:
                        ...
                        while (not bool(editItem["name"])) and (not bool(editItem["price"])) and (not bool(editItem["id"])):
                            ...
                            while not bool(editItem["name"]):
                                try:
                                    ...
                                    self.print('[red]❌ ชื่อที่คุณทำการแก้ไขนั้นเป็นชื่อซ้ำอยู่ในเมนูอาหารโปรดแก้ไขชื่อที่ไม่เหมือนกัน หรือ ห้ามใส่ชื่อว่างเปล่า![/]')
                                except UserWarning as err:
                                    self.print(err.__str__() , style='red')
                                else:
                            ...
                            # แสดงข้อความ
                            self.print(f'💬 รหัสสินค้าต้องตั้งเป็นเลขจำนวนเต็มจำนวน {self.PRODUCTCODE_LENGTH} ตัว')
    ...
    def __removeItem__(self) -> None:
        ...
        def deleteElements(param: str) -> None:
            ...
            if findIndex is None:
                ...
                self.print(f'[green bold]✓ ลบ [bold]"{foodName}"[/] ในรายการเมนูอาหารเสร็จสิ้น[/]')
    ...
    def EXECUTE(self) -> None:
        ...
        while self.__PROGRAMSTATUS__["programeIsRunning"]:
            ...
            try:
                ...
                if self.__isKeyword__(command):
                    ...
                    #* ซื้ออาหาร
                        ...
                        if self.__user__["AccessPermissions"]["SellFood"]:
                            ...
                            self.__accessDenide__(context="การสั่งซื้ออาหาร")
                    #* เพิ่มสินค้า
                        ...
                        if self.__user__["AccessPermissions"]["AddData"]:
                            ...
                            self.__accessDenide__(context="การเพิ่มเมนูอาหาร")
                    #* ลบสินค้า
                        ...
                        if self.__user__["AccessPermissions"]["DeleteData"]:
                            ...
                            self.__accessDenide__(context="การลบเมนูอาหาร")
                    #* แก้ไขสินค้า
                        ...
                        if self.__user__["AccessPermissions"]["ModifyData"]:
                            ...
                            self.__accessDenide__(context="การแก้ไขเมนูอาหาร")
                    #* ลบรายการสินค้าทั้งหมด
                        ...
                        if self.__user__["AccessPermissions"]["DeleteAllData"]:
                            ...
                            self.__accessDenide__(context="การลบรายการเมนูอาหารทั้งหมด")
                    #* แสดงกิจกรรมการทำงานต่างๆของโปรแกรม
                    elif command == "l" or command == "log":
                        if self.__user__["AccessPermissions"]["ViewLog"]:
                            self.__showLog__()
                        else:
                            self.__accessDenide__(context="การดู log ของโปรแกรม")
                    #* ออกจากบัญชี
                ...
                self.print("[bold underline red on grey0]Error:[/][red] คุณไม่ได้ป้อนคำสั่งโปรดพิมพ์คำสั่ง[/]")
            finally:
