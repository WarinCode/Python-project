...
class Date:
    ...
    def greeting(self) -> None:
        ...
        ต้องมีการรับค่า arguments มา 2 ตัวคือ hour และ userName
        """
        hour = self.time.hour
...
class Configuration(Date):
    ...
    def __login__(self) -> Dict[str , str]:   
        ...
        def loginFunction() -> Dict[str , bool | Dict[str , str]]:
            ...
            while not bool(userLogin["password"]):
                try:
                    ...
                    console.print(err.__str__() , style='red')
            # ส่งค่าเป็น dict
            return [bool(userLogin) , userLogin] # el1: สถานะข้อมูลของผู้ใช้งาน , el2: ข้อมูลผู้ใช้งานเป็น dictionary
        ...
        isValid = False
        saveUserData:Dict[str , str] = {} # บันทึก
        ...
        while not isValid:
            [isValid , saveUserData] = [*userVerification(*loginFunction())]
            if not isValid:
                counter += 1
    ...
    def __logout__(self) -> bool:
        ...
        if console.input('คุณต้องการออกจากบัญชีผู้ใช้งานนี้ [deep_pink1](y/n)[/] : ').lower().strip() == "y":
            ...
            self.__setUser__(isLogout=True) # set ข้อมูลผู้ใช้งานโปรแกรมเป็นค่าเริ่มต้น (ค่าว่างเปล่าตอนเริ่มโปรแกรม)
    ...
    def __showLog__(self) -> None:
        ...
        console.print(*self.self.__LOG__ , sep='\n' , style='blue')
        console.rule()
...
class Program(Configuration , Date): 
    ...
    def __createId__(self , length: int = 7) -> str:
        ...
        rand:List[str] = lambda: str(floor(random() * randint(1,10000))) # สุ่มเลขส่งคืนกลับมาเป็น string 
        while True:
    ...
    def __generateBill__(self , code:int , pay:int , result:int , order:List[Dict[str , str | int]] , change:int) -> None:
                ...
                while self.__PROGRAMSTATUS__["isContinue"]:
                    try:
                        ...
                        self.__allOrdersCode__.setdefault(self.__orderCode__ , {
                            ...
                            "pay": pay,
                            "change": abs(pay - self.__result__),
                            "total": self.__result__
                        }) 
                        #* สร้างบิลใบเสร็จ
                        self.__generateBill__(code=self.__orderCode__, order=self.__currentOrder__ , pay=pay , change=abs(pay - self.__result__) , result=self.__result__)
                        # เก็บ log
    ...
    def execute(self) -> None:
        """ (method หลัก) การดำเนินการทำงานหลักของโปรแกรม เป็นคำสั่งหลักในการรันโปรแกรม """
