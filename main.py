...
# สร้าง object console เพื่อนำไปใช้งานใน class ต่างๆ แสดงข้อความใน terminal แบบใส่ลูกเล่นต่างๆได้ 
console = Console()
...
class Register:
    ...
    def __login__(self) -> Dict[str , str]:   
        ...
        def loginFunction() -> Dict[str , Any]:
            ...
            while not bool(userLogin["nameOrEmail"]) or not bool(userLogin["password"]):
                while not bool(userLogin["nameOrEmail"]):
                    try:
                        nameOrEmail = console.input("ชื่อผู้ใช้งานหรืออีเมล : ").strip()
                        ...
                        console.print(f'[red]{err.__str__()}[/]')
                while not bool(userLogin["password"]):
                    try:
                        password = console.input("รหัสผ่าน : ").strip()
                        if isEmpty(password):
        ...
        while not userVerification(*loginFunction().values()):
            ...
            if counter > 4: # ถ้า login แล้วเกิดข้อผิดพลาดเกิน 4 ครั้งให้เสนอทางเลือกว่า สร้างบัญชีใหม่ก่อนแล้วค่อย login ทีหลัง
                if console.input('การ Login ล้มเหลวจำนวนหลายครั้งคุณต้องการ สมัครบัญชีผู้ใช้งานก่อนไหม (y/n) : ').lower().strip() == "y":
                    self.__createAccount__() # เรียกใช้ method สร้างบัญชีผู้ใช้งาน
                    break # ออกจาก loop นี้
        else: # หลังออกจาก loop
            console.print('[cyan]🔹 กำลัง login เข้าสู่ระบบ กรุณารอสักครู่ ████████░░░[/]')
            # sleep(3)
            console.print('[green bold]✓ login ผู้ใช้งานสำเร็จ[/]\n')
        # ส่งข้อมูลผู้ใช้งาน
    ...
    def __createAccount__(self) -> Dict[str , str]:
        ...
        while (not bool(newUser["name"])) or (not bool(newUser["password"])) or (not bool(newUser["position"])) or (not bool(newUser["email"])):
            ...
            while not bool(newUser["password"]):
                ...
                try:
                    ...
                    if isEmpty(password):
                        ...
                        confirmPassword = console.input("ยืนยันรหัสผ่าน : ").strip()
            ...
            console.print('[cyan]กำลังสร้างบัญชีผู้ใช้งาน กรุณารอสักครู่ ████████░░░ [/]')
            # sleep(3)
            console.print('[green bold]✓ สร้างบัญชีผู้ใช้งานสำเร็จโปรด Login เพื่อเข้าใช้งานโปรแกรม[/]')
            console.print('🔹[blue] กลับไปที่หน้า Login[/]')
            Users().addUser(newUser=newUser) # เพื่มข้อมูลผู้ใช้งานคนใหม่
    ...
    def __logout__(self , callBackFunction: Any) -> bool:
        ...
        if console.input('คุณต้องการออกจากบัญชีผู้ใช้งานนี้ (y/n) : ').lower().strip() == "y":
            ...
            console.print('[cyan]กำลังนำคุณออกจากบัญชี ...[/]')
            # sleep(3)
            console.print('[blue]คุณออกจากบัญชีนี้เรียบร้อย[/]')
        else:
            console.print('[orange1]⚠ คุณยกเลิกการออกจากบัญชีนี้[/]')
        return status
    ...
    def __getUser__(self , callBackFunction) -> Dict[str , str | Union[str , Dict[str , str]]]:
        ...
        while not bool(user):
            try:
                ...
                selected = int(console.input('โปรดพิมพ์ตัวเลือก : '))
                # เลือก login
                if selected == 1:
                    user = self.__login__()
...
class Date:
    ...
    def greeting(self , h: int) -> None:
        ...
        # แสดงข้อความ
        console.print(f'🙏 {hi} คุณ {self.__user__["name"]} วันนี้ วัน{self.days[self.now.date().weekday()]} ที่ {self.now.date().day} เดือน {self.months[self.now.date().month - 1]} ปี พ.ศ. {self.year} ({self.today})')
...
class Program(Configuration , Date): 
    ...
    __PROGRAMSTATUS__ = {
       ...
       "isFirstCreateTable": None # สถานะการสร้างตารางครั้งแรก -> True: สร้างครั้งแรก , False: ไม่ได้สร้างครั้งแรก (ใช้แค่บางตารางเท่านั้น)
    }
    ...
    def showMenu(self):
        ...
        # เอกสารประกอบการใช้งาน API: https://rich.readthedocs.io/en/latest/tables.html
        ...
        console.print(f'🌟 ตอนนี้ไม่มีรายการสินค้าใดๆโปรดเพิ่มสินค้าก่อนแสดงรายการเมนู!') if self.__menu__.__len__() == 0 else console.print('\n' , self.__menuTable__ , '\n')
    ...
    def __showLog__(self) -> None:
        console.rule(title='[yellow bold]บันทึกของโปรแกรม[/]')
    ...
    def __foodOrdering__(self) -> None:  
        # แสดงเส้นขั้น
        console.rule(title='สั่งอาหาร' , style='grey93')
        # แสดงแจ้งเตือน
        ...
        while self.__PROGRAMSTATUS__["isInvokeMethods"]:
            try:
                ...
                # ยกเลิกอาหารที่สั่ง
                    ...
                    console.print('[green]✓ ลบรายการ order ที่ทำการสั่งไปเรียบร้อย[/]')
                    # เก็บ log
                ...
                # แสดง order ที่สั่งไป
                    ...
                    if self.__currentOrder__ == []:
                        ...
                        totalAmount = 0
                        count = 0
                        console.print('อาหารที่คุณสั่งไปคือ : ')
                        for item in self.__currentOrder__:
                            count += 1
                            totalAmount += item["amount"]
                            console.print(f'{count}.) {item["name"]} จำนวน {item["amount"]} อย่าง')
                        console.print(f'จำนวนทั้งหมด {totalAmount}')
                ...
                #* เช็คชื่ออาหารหรือรหัสสินค้าว่าอยู่ใน list หรือไม่
                    ...
                    while self.__PROGRAMSTATUS__["isContinue"]:
                        try:
                            ...
                            console.print("[red]❌ โปรดใส่เป็นเลขจำนวนเต็มเท่านั้น![/]")
                        except UserWarning as err:
                            console.print(f'[red]{err.__str__()}[/]')
                        else:
                            ...
                            if foodName not in self.__shoppingList__: 
                                if self.__PROGRAMSTATUS__["isError"]: pass
                                    ...
                                    # console.print(self.__currentOrder__)
                                    # เก็บ log
    ...
    def __addItem__(self) -> None:
        # แสดงเส้นขั้น
        console.rule(title='เพิ่มเมนูอาหาร' , style='grey93')
        # แสดงแจ้งเตือน
        self.notify('เพื่อออกจาการเพิ่มสินค้า' , None)
        # infinity loop จนกว่าจะพิมพ์คำสั่ง "e" หรือ "end" เพื่อออกจาก loop
        while self.__PROGRAMSTATUS__["isInvokeMethods"]:
            try:
                newItem = console.input('ชื่ออาหารใหม่ : ').strip().lower()
                # ออกจาการทำงานของ method
                    ...
                    try:
                        ...
                        # ห้ามตั้งน้อยกว่าราคาที่ตั้งไว้ 
                            ...
                            if newItem not in self.__foodList__:
                                ...
                                console.print(f'[green bold]✓ เพิ่มเมนูอาหารใหม่เสร็จสิ้น[/]')
                        ...
                        console.print('[red]❌ ไม่สามารถตั้งราคาสินค้าได้ราคาสินค้าต้องตั้งเป็นตัวเลขจำนวนเต็มเท่านั้น![/]')
                    else: 
                        self.__PROGRAMSTATUS__["isContinue"] = False
    ...
    def __editItem__(self) -> None:
        # แสดงเส้นขั้น
        console.rule(title='แก้ไขเมนูอาหาร' , style='grey93')
        # แสดงแจ้งเตือน
        ...
        while self.__PROGRAMSTATUS__["isInvokeMethods"]:
            try:
                ...
                # ถามข้อมูล
                item = console.input('ใส่ชื่ออาหารหรือรหัสสินค้าเพื่อทำการแก้ไข : ')
                        ...
                        while (not bool(editItem["name"])) and (not bool(editItem["price"])) and (not bool(editItem["id"])):
                            ...
                            while not bool(editItem["name"]):
                                try:
                                    ...
                                    console.print('[red]❌ ชื่อที่คุณทำการแก้ไขนั้นเป็นชื่อซ้ำอยู่ในเมนูอาหารโปรดแก้ไขชื่อที่ไม่เหมือนกัน หรือ ห้ามใส่ชื่อว่างเปล่า![/]')
                                except UserWarning as err:
                                    console.print(f'[red]{err.__str__()}[/]')
                                else:
                            ...
                            while not bool(editItem["price"]):
                                try:
                                    ...
                                    console.print('[red]❌ ราคาสินค้าและรหัสสินค้าต้องตั้งเป็นตัวเลขจำนวนเต็มเท่านั้น![/]')
                                except UserWarning as err:
                                    console.print(f'[red]{err.__str__()}[/]')
                                else:
                                    editItem["price"] = changePrice
                            # แสดงข้อความ
                            console.print(f'💬 รหัสสินค้าต้องตั้งเป็นเลขจำนวนเต็มจำนวน {self.PRODUCTCODE_LENGTH} ตัว')
    ...
    def __removeItem__(self) -> None:
        # แสดงเส้นขั้น
        console.rule(title='ลบรายการเมนูอาหาร' , style='grey93')
        # แสดงแจ้งเตือน
        ...
        def deleteElements(param: str) -> None:
            ...
            if findIndex is None:
                console.print(f'[red]❌ ไม่พบ [bold]"{param}"[/] อยู่ในเมนูอาหาร[/]')
            else:
                ...
                console.print(f'[green bold]✓ ลบ [bold]"{foodName}"[/] ในรายการเมนูอาหารเสร็จสิ้น[/]')
            ...
            try:
                ...
                # แสดงรายการเมนู    
                    ...
                    if ',' in item or ',' in [*item]: # ถ้าใส่ , ให้ทำเงิอนไขนี้
                        ...
                        # ลบสินค้าที่ละชิ้น
                        if console.input(f'[dark_orange]คุณแน่ใจว่าต้องการลบสินค้าเหล่านี้ออกจากรายการเมนูอาหารของร้านอาหาร (y/n) : [/]').lower().strip() == "y":        
                    ...
                    # เมื่อมีข้อมูลให้ลบออก
                        ...
                        if console.input(f'[dark_orange]คุณแน่ใจว่าต้องการลบ [bold]"{item}"[/] ออกจากรายการเมนูอาหารของร้านอาหาร (y/n) : [/]').lower().strip() == "y":
                            deleteElements(item)
                        else:
                            console.print('คุณยกเลิกการลบ' , style='dark_orange')  
                    elif item == "":
    ...
    def __deleteMenu__(self) -> None:
        # แสดงเส้นขั้น
        console.rule(title='ลบเมนูทั้งหมด' , style='grey93')
        if console.input('คุณแน่ใจว่าต้องการลบสินค้าทั้งหมดถ้าต้องการให้พิมพ์ "y" แต่โปรดรู้ไว้ข้อมูลสินค้าจะถูกลบถาวรและไม่สามารถกู้คืนได้ (y/n): ').lower().strip() == "y":
            self.__log__(typeOfLog=self.DELALL)
            self.__menu__.clear()
            console.print('[green]✓ ลบรายการเมนูอาหารทั้งหมดเสร็จสิ้นเรียบร้อย[/]')
        else: console.print('[magenta]❗ คุณยกเลิกการดำเนินการลบสินค้าทั้งหมด[/]')
...
อาหารที่สั่งบ่อยหรือสั่งเยอะที่สุดในวันนี้คือ [yellow bold]\"{mo}\"[/]"""
    ...
    def __exitProgram__(self) -> None:
        ...
        self.__allOrders__.__len__() != 0 and console.print(self.__conclusion__(total=self.__totalMoney__ , orders=self.__allOrders__))
    ...
    def EXECUTE(self) -> None:
        ...
        while self.__PROGRAMSTATUS__["programeIsRunning"]:
            ...
            try:
                command = console.input("[medium_turquoise]พิมพ์คำสั่งเพื่อดำเนินการต่อไป >>> [/]")
                ...
                if self.isKeyword(command):
                    ...
                    #* ออกจากบัญชี
                        ...
                        if self.__logout__(callBackFunction=self.__log__): # callBackFunction คือ parameter ใน method logout โดยจะส่ง method เพื่อนำไปใช้งาน
                            self.__PROGRAMSTATUS__["isWorking"] = False
                            # ให้ login ใหม่
                            user = super().__getUser__(callBackFunction=self.__log__) # รอรับข้อมูลผู้ใช้งาน
                ...
                console.print("[bold underline red on grey0]Error:[/][red] คุณไม่ได้ป้อนคำสั่งโปรดพิมพ์คำสั่ง[/]")
            finally:
                self.__PROGRAMSTATUS__["isWorking"] and console.print('โปรดเลือกพิมพ์คำสั่ง')
