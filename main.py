...
class ReadWrite:
    ...
    def write(data:List[str | Dict[str,str]], isArray:bool = False, path:str = r'C:\Users\ACER USER5949486\Desktop\Python-project\data\menu.py' , mode:str ='w') -> None:
        ...
        #? มีการรับค่า parameters มา 3 ตัวคือ data , path และ mode
        ...
        #? mode คือการเลือกว่าจะดำเนินการอย่างไรในการเปิดไฟล์ ในที่นี้ set default parameter ให้เป็น w คือเขียนไฟล์ ถ้าต้องการเพิ่มเนื้อหาข้อมูลของไฟล์ให้เปลี่ยน mode เป็น a  
        try:
...
class Date:
    ...
    def greeting(self , userName:str) -> None:
        """ method ทักทายผู้ใช้งานและบอกเวลา """
        hour = self.time.hour 
        hi = ''
...
class Configuration(Date):
    ...
    __user__ = {
        ...
        "email": None,
        "position": None,
        "AccessPermissions": {} # สิทธิ์การเข้าถึงของคำสั่งโดยขึ้นอยู่กับตำแหน่งงานของผู้ใช้งานในร้านอาหารนั้น
    }
    ...
    def __login__(self) -> Dict[str , str]:   
        ...
        def loginFunction() -> Dict[str , bool | Dict[str , str]]:
            ...
            while not bool(userLogin["nameOrEmail"]):
                try:
                    ...
                    if isEmpty(nameOrEmail):
                        ...
                        userLogin.update(nameOrEmail=nameOrEmail) # อัปเดตค่า value
            ...
            while not bool(userLogin["password"]):
                try:
                    ...
                    if isEmpty(password):
                        ...
                        userLogin.update(password=password)
                except Exception as err:
                    console.print(err.__str__() , style='red')
        ...
        def userVerification(status: bool , validateUser: Dict[str , str]) -> bool:
            ...
            #* ถ้าค่า True แปลว่ามีข้อมูลผู้ใช้งานมีอยู่ในระบบ
            isCorrect = { 
                ...
                if status:
                    ...
                    for user in self.__usersData__: # ดึง element (dict)แต่ละอันออกมาเช็คว่าตรงกันไหม
                        ...
                        if validateUser["nameOrEmail"] == user["name"]:
                            # ถ้ามีข้อมูลในไฟล์ที่เก็บให้ค่าสถานะเป็น True
                            isCorrect["name"] = True
                        # ตรวจสอบ อีเมล
                        ...
                        if ((isCorrect["name"] or isCorrect["email"]) and isCorrect["password"]) or validateUser["nameOrEmail"] == 'root': 
                            ...
                            if validateUser["nameOrEmail"] == 'root':
                                ...
                                validateUser["email"] = 'root@dev'
                                validateUser["position"] = 'ผู้ดูแลระบบและพัฒนาระบบ'
                        ...
                        # ชื่อผู้ใช้ถูกต้อง หรือ อีเมลถูกต้อง แต่ รหัสผ่านผิดให้แสดง error
                        ...
                        # ชื่อผู้ใช้ หรือ อีเมล ผิด แต่ รหัสผ่านถูกต้องให้แสดง error 
    ...
    def __createAccount__(self) -> Dict[str , str]:
        ...
        while not bool(newUser["name"]):
            try:
                ...
                if isEmpty(name):
                    ...
                    newUser.update(name=name) # key = ค่า value ใหม่
            except Exception as err:
                console.print(err.__str__() , style='red')
        ...
        while not bool(newUser["email"]):
            try:
                ...
                console.print(f'❌ "{email}" ไม่ใช้อีเมลโปรดลองใหม่อีกครั้ง' , style='red')
        ...
        while not bool(newUser["position"]):
            try:
                ...
                newUser.update(position=selectedPosition)
    ...
    def __getUser__(self) -> Dict[str , str | Dict[str , str]]:
        """ method ในการรับข้อมูลผู้ใช้งานจากการ login  """
        # ข้อมูลผู้ใช้งาน
        user: None | Dict[str , str | Dict[str , str]] = None
        # แสดงข้อความ
        console.line()
    ...
    def __setUser__(self , user:Dict[str , str] | None , isLogout:bool = False) -> None:
        ...
        user คือ ข้อมูลผู้ใช้งานที่ได้จากการ login ข้อมูลจะเป็น dict แต่ถ้าไม่มีให้ส่งเป็น None มาได้ 
        isLogout คือ สถานะที่จะนำบัญชีออกจากโปรแกรมจะต้องส่ง True มาเพื่อนำผู้ใช้งานออกจากโปรแกรม """
    ...
    def __setPermissions__(self , user:Dict[str , str | Dict[str , str]]) -> None:
    ...
    def __log__(self , text:str = "" , typeOfLog: None | str = None , item: None | List[str | int] = None) -> None:
        ...
        มี default parameters 2 ตัวมีค่าเป็น None
        """
...
class Program(Configuration , Date): 
    ...
    def __searchReferentCode__(self) -> None:
        ...
        # แสดงเส้นขั้น
        console.rule(title='ค้นหา order' , style='grey93')
        # แสดงแจ้งเตือน
        ...
        def resetOrder(isCancel:bool = False) -> None:
            if isCancel:
                if isCancel and self.__currentOrder__ == []:
                    ...
                    console.print('[green]✓ ยกเลิกรายการ order ที่ทำการสั่งไปเรียบร้อย[/]')
                    # เก็บ log
        ...
        while self.__PROGRAMSTATUS__["invokeMethods"]:
            try:
                ...
                # แสดงแจ้งเตือนอีกครั้ง
                    ...
                    # ใส่ข้อมูลไม่ถูกต้อง ไม่มีข้อมูลอยู่ใน menu
                    if idx is None: 
    ...
    def __deleteMenu__(self) -> None:
        ...
        if console.input('คุณแน่ใจว่าต้องการลบสินค้าทั้งหมดถ้าต้องการให้พิมพ์ "y" แต่โปรดรู้ไว้ข้อมูลสินค้าจะถูกลบถาวรและไม่สามารถกู้คืนได้ [deep_pink1](y/n)[/] : ').lower().strip() == "y":
    ...
    def __conclusion__(self , total: List[int] , orders: List[Dict[str , int]]) -> Tuple[str]:
        ...
        for item in orders:
            ...
            li = [foodName for i in range(amount)] 
...
# # เรียกใช้ method จาก object เพื่อดำเนินการทำงาน                                                                                                                                                                           
# program.execute() 

#? วิธีที่ 2 
data = { # อ่านข้อมูลจากไฟล์แล้วมาเก็บไว้ใน property 
