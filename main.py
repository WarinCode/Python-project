...
#* ไฟล์ code ทั้งหมดของ project อยู่ที่ github -> https://github.com/VarinCode/Python-project
#* โดยแบ่งได้เป็น 3 branches ได้แก่:
#* main branch : เป็น branch หลักของโปรเจค เขียนด้วยรูปแบบ oop พัฒนาอยู่ในปัจจุบัน โปรแกรมมีฟีเจอร์ต่างๆพร้อมให้ใช้งาน
#* prototype branch : code รุ่นแรกที่ถูกพัฒนาภายในไม่กี่วันเป็นแค่การทดลองและทดสอบฟีเจอร์ที่ต้องมีและจะนำมาใช้ภายในโปรเจค สามารถใช้งานฟีเจอร์หลักๆได้ แต่ยังคงมี bug
#* type branch : เหมือน code ของ main branch ทุกอย่างแต่จะเขียน type เพิ่มใน ตัวแปร , object , parameters และ methods
...
class Configuration(Date):
    ...
    def __createAccount__(self) -> None:
        ...
        while not bool(newUser["position"]):
            try:
                select = console.input("ตำแหน่งงานหรือหน้าที่ของคุณคือ : ").strip()
                if isEmpty(select):
    ...
    def __logout__(self) -> bool:
        ...
        confirm = False # สถานะออกจากบัญชี (True : ออกจากบัญชี / False : ไม่ได้ออกจากบัญชี)
...
class Program(Configuration , Date): 
    ...
    def __generateBill__(self , code: int , pay: int , total: int , order: List[Dict[str , str | int]] , change: int) -> None:
        ...
        for n , item in enumerate(iterable=order , start=1): # loop ตามจำนวนข้อมูลที่ส่งมาจะได้ dictionary ที่เก็บอยู่ใน list
            ...
            details.add_row(f'{n}' , item['name'] , f'{item["amount"]}' , f'{item["price"]}' , f'{item["total"]:,} บาท')
    ...
    def __editProduct__(self) -> None:
        ...
        while self.__PROGRAMSTATUS__["invokeMethod"]:
            try:
                ...
                # ถามข้อมูล
                product = console.input('ใส่ชื่ออาหารหรือรหัสสินค้าเพื่อทำการแก้ไข : ').lower().strip()
                # ออกจาการทำงานของ method
    ...
    def __deleteProduct__(self) -> None:
        ...
        while self.__PROGRAMSTATUS__["invokeMethod"]:
            ...
            try:
                product = console.input('ใส่ชื่ออาหารหรือรหัสสินค้าเพื่อทำการลบ : ').lower().strip() 
                # ออกจาการทำงานของ method
                if product == 'e' or product == 'end':  
                    self.__PROGRAMSTATUS__["invokeMethod"] = False
                # แสดงรายการเมนู    
                ...
                # แสดงแจ้งเตือนอีกครั้ง
                    ...
                    if ',' in product or ',' in [*product]: # ถ้าใส่ , ให้ทำเงิอนไขนี้
                        formatList = product.split(',') # ลบ , ออกจะได้ ข้อมูลเป็น list
                        # จัดระเบียบข้อความ
